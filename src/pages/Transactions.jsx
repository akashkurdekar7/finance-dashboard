...
const TransactionsTable = () => {
  ...
  const handleExportJSON = () => {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(sortedTransactions, null, 2));
  ...
  const SortIcon = ({columnKey}) => {
    if (sortConfig.key !== columnKey)
      return (
        <ArrowUpDown
          size={14}
          className="ml-1 inline opacity-40 group-hover:opacity-100"
        />
      );
    if (sortConfig.direction === "asc")
      return <ArrowUp size={14} className="ml-1 inline text-brand-primary" />;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium tracking-tight">
              Detailed log of all financial movements
            </p>
          </div>
        </div>
            ...
            <thead>
              <tr className="bg-slate-900/50 border-b border-slate-700/50">
                <th
                  ...
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400 italic cursor-pointer group hover:bg-slate-800/50 transition-colors">
                  Date <SortIcon columnKey="date" />
                </th>
                <th
                  onClick={() => requestSort("category")}
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400 cursor-pointer group hover:bg-slate-800/50 transition-colors">
                  Category <SortIcon columnKey="category" />
                </th>
                <th
                  onClick={() => requestSort("amount")}
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400 text-right cursor-pointer group hover:bg-slate-800/50 transition-colors">
              ...
              {sortedTransactions.length > 0 ? (
                sortedTransactions.map((t) => (
                  <tr
                    ...
                    <td
                      className={`px-6 py-5 text-right font-bold text-lg tracking-tighter ${t.type === "income" ? "text-brand-primary" : "text-slate-100"}`}>
                      {t.type === "income" ? "+" : "-"} ₹
                      {Number(t.amount).toLocaleString()}
                    </td>
                    <td className="px-6 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${t.type === "income" ? "bg-brand-primary/10 text-brand-primary border border-brand-primary/20" : "bg-red-400/10 text-red-400 border border-red-400/20"}`}>
                        {t.type}
                      </span>
                    </td>
                ...
                <tr>
                  <td
                    colSpan={
                      permissions.canEdit || permissions.canDelete ? 5 : 4
                    }
                    ...
                    <div className="flex flex-col items-center gap-3 opacity-40">
                      ...
                      <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">
                        Check applied filter configuration
                      </p>
                    </div>
                  </td>
                </tr>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <CustomSelect
                      ...
                      icon={ArrowRightLeft}
                      onChange={(val) =>
                        setEditingTxn({...editingTxn, type: val})
                      }
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingTxn({...editingTxn, amount: e.target.value})
                    }
                ...
                <button
                  type="submit"
                  className="w-full py-5 bg-brand-primary text-slate-950 rounded-[1.5rem] font-black uppercase tracking-[0.3em] shadow-2xl shadow-brand-primary/20 hover:scale-[1.02] active:scale-95 transition-all text-xs">
                  Save Record
                </button>
              </form>
            </motion.div>
