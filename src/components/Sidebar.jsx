...
//     return (
//         <motion.div
//            className="w-[260px] h-full bg-slate-950 border-r border-slate-800 flex flex-col gap-10"
//             // initial={{ width: 100 }}
...
const Sidebar = () => {
  ...
  return (
    <div className="w-[260px] h-full bg-slate-950 border-r border-white/5 flex flex-col justify-between p-4">

      {/* 🔥 LOGO */}
      <div>
        <div className="flex items-center gap-3 px-2 py-4 mb-6">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-brand-primary to-brand-accent flex items-center justify-center shadow-lg">
            ...
            <NavLink key={item.path} to={item.path}>
              {({ isActive }) => (
                <motion.div
                  ...
                  ${
                    isActive
                      ? "bg-gradient-to-r from-brand-primary/20 to-brand-accent/10 text-white border border-white/10 shadow-inner"
                      : "text-slate-400 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <div className={`${isActive ? "text-brand-primary" : ""}`}>
                    {item.icon}
                  </div>
                  ...
                  {/* 🔥 ACTIVE GLOW BAR */}
                  {isActive && (
                    <div className="ml-auto w-1.5 h-5 rounded-full bg-brand-primary shadow-[0_0_10px_var(--brand-primary)]" />
                  )}
                </motion.div>
              )}
            </NavLink>
      ...
      {/* 🔥 ROLE CARD */}
      <div className="mt-6">
        <div className="p-4 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-xl shadow-inner">
